...
class NoticiasSpider(scrapy.Spider):
    ...
    def parse_noticia(self, response):
        item = Noticia()
        item['titulo'] = response.css('h1.title::text').get().replace('\n', '').strip()  #tag h2 ou h3; classe title texto (inner text)
        item['descricao'] = response.css('h2.description::text').get().replace('\n', '').strip()
        textos = response.css('section.content p span::text, section.content p::text').extract()
